...
const AdminLogin = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if(input.email !== import.meta.env.VITE_ADMIN_EMAIL || input.password !== import.meta.env.VITE_ADMIN_PASS){
    ...
    return (
        <div className='w-full h-[92vh] flex text-white items-center justify-center' style={ { backgroundImage: "url('/img/loginBg.jpeg')", backgroundSize: "cover", } }>
            <div className='w-full md:w-1/3 sm:w-1/2 m-auto px-10 py-4'>
                ...
                <div>
                    <button onClick={ () => navigate("/login") } className='bg-sky-600 relative -right-52 mt-7 py-2 px-4 rounded-md hover:bg-sky-800'>User Login</button>
                </div>
